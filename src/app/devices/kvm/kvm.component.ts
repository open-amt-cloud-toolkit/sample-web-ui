...
import {
  KVMComponent,
  IDERComponent,
} from '@open-amt-cloud-toolkit/ui-toolkit-angular';
...
export class KvmComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.startSocketSubscription =
      this.devicesService.connectKVMSocket.subscribe((data: boolean) => {
        this.init();
  ...
  init(): void {
    ...
    this.getPowerState(this.deviceId)
      .pipe(
        ...
        switchMap((result) =>
          result === null ? of() : this.getRedirectionStatus(this.deviceId),
        ),
        switchMap((result: RedirectionStatus) =>
          this.handleRedirectionStatus(result),
        ),
        switchMap((result) => (result === null ? of() : this.getAMTFeatures())),
        switchMap((results: AmtFeaturesResponse) =>
          this.handleAMTFeaturesResponse(results),
        ),
        ...
        switchMap((result: boolean | null) =>
          this.handleUserConsentDecision(result),
        ),
        switchMap((result: any | userConsentResponse) =>
          this.handleUserConsentResponse(result),
        ),
      )
      .subscribe()
      .add(() => {
  ...
  cancelEnableKvmResponse(result?: boolean): void {
    ...
    if (!result) {
      this.displayError(
        $localize`KVM cannot be accessed - request to enable KVM is cancelled`,
      );
    } else {
      this.displayError(
        $localize`KVM cannot be accessed - failed to enable KVM`,
      );
    }
  ...
  handleUserConsentResponse(
    ...
    if (result === true || result.Body?.ReturnValue === 0) {
      ...
      this.displayError(
        $localize`KVM cannot be accessed - failed to request user consent code`,
      );
      return of(null);
    }
  }
  ...
  afterUserConsentDialogClosed(data: userConsentData): void {
    ...
    if (response.error != null) {
      this.displayError(
        `Unable to send code: ${response.error.Body.ReturnValueStr as string}`,
      );
      this.isLoading = false;
    } else {
      if (environment.cloud) {
        ...
        const method = response.XMLName.Local;
  ...
  cancelOptInCodeResponse(result: userConsentResponse): void {
    ...
    if (result.Body?.ReturnValue === 0) {
      this.displayError(
        $localize`KVM cannot be accessed - requested user consent code is cancelled`,
      );
    } else {
      this.displayError(
        $localize`KVM cannot be accessed - failed to cancel requested user consent code`,
      );
    }
  }
  ...
  sendOptInCodeResponse(result: userConsentResponse): void {
    if (result.Body?.ReturnValue === 0) {
      ...
      this.displayError(
        $localize`KVM cannot be accessed - unsupported user consent code`,
      );
      this.getAMTFeatures();
    } else {
      this.isLoading = false;
      this.displayError(
        $localize`KVM cannot be accessed - failed to send user consent code`,
      );
    }
  }
  ...
  reqUserConsentCode(guid: string): Observable<userConsentResponse> {
    return this.devicesService.reqUserConsentCode(guid).pipe(
      catchError((err) => {
        ...
        this.isLoading = false;
        this.displayError(
          $localize`Error requesting user consent code - retry after 3 minutes`,
        );
        return of(err);
      }),
    );
    ...
    this.devicesService
      ...
      .subscribe((data: userConsentResponse) => {
        if (data.Body?.ReturnValue === 0) {
          this.displayWarning(
            $localize`KVM cannot be accessed - previously requested user consent code is cancelled`,
          );
        } else {
          this.displayError(
            $localize`KVM cannot be accessed - failed to cancel previous requested user content code`,
          );
        }
      });
  }
