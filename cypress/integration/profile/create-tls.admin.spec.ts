...
describe('Test Profile Page', () => {
  ...
  it('should create a profile with random AMT password and static MEBx password with TLS config', () => {
    cy.myIntercept('POST', 'profiles', {
      statusCode: apiResponses.profiles.create.success.code,
      body: apiResponses.profiles.create.success.response
    }).as('post-profile2')
    ...
    cy.enterProfileInfo(
      profileFixtures.happyPathTlsAdmin.profileName,
      profileFixtures.happyPathTlsAdmin.activation,
      true,
      false,
      profileFixtures.happyPathTlsAdmin.dhcpEnabled,
      profileFixtures.happyPathTlsAdmin.connectionMode,
      profileFixtures.happyPathTlsAdmin.tlsConfig
    )
    cy.get('input').get('[formControlName=amtPassword]').should('not.be.visible')
    ...
    cy.wait('@post-profile2').then((req) => {
      ...
      cy.get('mat-cell').contains(profileFixtures.happyPathTlsAdmin.profileName)
