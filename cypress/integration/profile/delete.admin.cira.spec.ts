...
describe('Test Profile Page', () => {
  ...
  it('deletes the admin control mode cira config profile', () => {
    ...
    // Delete profile
    cy.get('mat-row').contains(profileFixtures.happyPathAdmin.profileName).parent().contains('delete').click()
    cy.get('button').contains('Yes').click()
    cy.wait('@delete-profile')
    cy.wait('@get-profiles4')

    cy.contains(profileFixtures.happyPathAdmin.profileName).should('not.exist')
  })
})
