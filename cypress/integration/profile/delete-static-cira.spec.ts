...
describe('Test Profile Page', () => {
  ...
  it('deletes the default static+cira profile', () => {
    // Stub the requests
    ...
    // Delete profile
    cy.get('mat-row').contains(profileFixtures.happyPathStaticCIRA.profileName).parent().contains('delete').click()
  ...
  it('deletes the default profile with static+cira and random password', () => {
    ...
    // Delete profile
    cy.get('mat-row').contains(profileFixtures.happyPathStaticCIRARandomPassword.profileName).parent().contains('delete').click()
    cy.get('button').contains('Yes').click()
    cy.wait('@delete-profile')
    cy.wait('@get-profiles4')
