...
describe('Test Profile Page', () => {
  ...
  it('deletes the static profile with tls configuration', () => {
    ...
    // Delete profile
    cy.get('mat-row').contains(profileFixtures.happyPathTlsRandomMebxPwd.profileName).parent().contains('delete').click()
    cy.get('button').contains('Yes').click()
    cy.wait('@delete-profile')
    cy.wait('@get-profiles4')

    // Check that the config was deleted properly
    cy.contains(profileFixtures.happyPathTlsRandomMebxPwd.profileName).should('not.exist')
  })
})
