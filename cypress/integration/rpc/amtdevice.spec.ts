describe('Successful execution of pre-provisioning on amtdevice', () => {
  ...
  it('Control Mode is pre-provisioning with Deacivate Device', { execTimeout: 90000 }, () => {
    cy.exec('docker run --device=/dev/mei0 intel/oact-rpc-go:latest --url deactivate -u wss://cicdoact.eastus.azurecontainer.io:8443/activate', { failOnNonZeroExit: false }).then((result) => {
  ...
  it('Control Mode is pre-provisioning with Activate Device', { execTimeout: 90000 }, () => {
    cy.exec('docker run --device=/dev/mei0 intel/oact-rpc-go:latest activate -u wss://cicdoact.eastus.azurecontainer.io:8443/activate',{ failOnNonZeroExit: false }).then((result) => {
