describe('Successful execution of pre-provisioning on amtdevice', () => {
  ...
  it('Control Mode is pre-provisioning with Activate Device', () => {
    cy.exec('docker run --device=/dev/mei0 intel/oact-rpc-go:latest activate -u wss://cicdoact.eastus.azurecontainer.io:8443/activate -n --profile happyPath',{ failOnNonZeroExit: false }).then((result) => {
