describe('Activating AMT device', () => {
  it('Activating AMT device', { execTimeout: 240000 }, () => {
    cy.exec('docker run --device=/dev/mei0 intel/oact-rpc-go:latest activate -u ${profileName}/activate -v -n --profile ${profileName}', { failOnNonZeroExit: false }).then((result) => {      
